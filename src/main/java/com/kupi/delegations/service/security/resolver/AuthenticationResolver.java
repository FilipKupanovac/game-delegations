package com.kupi.delegations.service.security.resolver;
...
public class AuthenticationResolver {
    ...
    public Authentication authenticate(HttpServletRequest request) {
        ...
        List<SimpleGrantedAuthority> grantedAuthorities = user.getUserRoles().stream().map(
                SimpleGrantedAuthority::new).toList();
        UserDetailsImpl userDetails = new UserDetailsImpl(user.getEmail(), user.getEncryptedPassword(), user.getUuid(), grantedAuthorities);
